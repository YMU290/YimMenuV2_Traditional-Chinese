...
namespace YimMenu::Features
{
	class NpcAutoDrive : public LoopedCommand
	{
		...
		bool ValidateSetupCandidate(Ped player, Vehicle vehicle, std::optional<int>& passengerSeat)
		{
			if (!player || !vehicle)
			{
				SetFailure(FailureReason::NoVehicle, TR("Enter the driver seat of a supported car."));
				return false;
			}
		...
		virtual void OnTick() override
		{
			...
			if (routeResult == AutoDriveInternal::RouteResult::DestinationReached)
			{
				...
				Notifications::Show(
				    TR("NPC Auto Drive"),
				    TR("Destination reached. NPC Auto Drive disabled; please take control."),
	...
	static NpcAutoDrive _NpcAutoDrive{
	    "npcautodrive",
	    TR("NPC Auto Drive"),
	    TR("Moves you to a passenger seat and lets an NPC drive to your waypoint or roam")};
}
