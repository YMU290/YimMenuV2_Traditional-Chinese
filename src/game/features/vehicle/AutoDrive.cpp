...
namespace YimMenu::Features
{
	class AutoDrive : public LoopedCommand
	{
		...
		bool ValidateDriver(Ped driver, Vehicle vehicle)
		{
			...
			{
				m_Route.ClearTask();
				SetFailure(FailureReason::NoVehicle, TR("Enter the driver seat of a supported road vehicle."));
				return false;
			}
			...
			{
				m_Route.ClearTask();
				SetFailure(FailureReason::PlayerDead, TR("Auto Drive is waiting for the player to respawn."));
				return false;
			}
			...
			{
				m_Route.ClearTask();
				SetFailure(FailureReason::NotDriver, TR("Move to the driver seat to use Auto Drive."));
				return false;
			}
			...
			{
				m_Route.ClearTask();
				SetFailure(FailureReason::UnsupportedVehicle, TR("This vehicle type does not support Auto Drive."));
				return false;
			}
			...
			{
				m_Route.ClearTask();
				SetFailure(FailureReason::VehicleUndriveable, TR("The current vehicle cannot be driven."));
				return false;
			}
			...
			{
				m_Route.ClearTask();
				SetFailure(FailureReason::NoControl, TR("Waiting for network control of the current vehicle."));
				return false;
			}
		...
		virtual void OnTick() override
		{
			...
			const auto manualInput = m_Route.HasTask() || isCurrentDriver ? GetManualDrivingInput() : ManualInput::None;
			...
			if (routeResult == AutoDriveInternal::RouteResult::DestinationReached)
			{
				...
				Notifications::Show(
				    TR("Auto Drive"),
				    TR("Destination reached. Auto Drive disabled; please take control."),
	...
	static AutoDrive _AutoDrive{
	    "autodrive",
	    TR("Auto Drive"),
	    TR("Drives to your waypoint using roads, or roams when no waypoint is set")};
}