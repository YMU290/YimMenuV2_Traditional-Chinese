...
namespace YimMenu::Features
{
	namespace DiamondCasinoHeist
	{
		...
		class SkipDrilling : public Command
		{
			...
			{
				if (auto thread = Scripts::FindScriptThread("fm_mission_controller"_J))
					*ScriptLocal(thread, 10567).At(7).As<int*>() = *ScriptLocal(thread, 10551).At(37).As<int*>();
			}
		};
		...
		static RemoveCameras _DiamondCasinoHeistRemoveCameras{"diamondcasinoheistremovecameras", TR("Remove Cam"), TR("Removes all cameras")};
		static RemoveKeycard _DiamondCasinoHeistKeycard{"diamondcasinoheistkeycard", TR("Remove Keycard"), TR("Removes keycard")};
		static Auto_Grab _DiamondCasinoHeistAutoGrab{"diamondcasinoheistautograb", TR("Auto-Grab"), TR("Casino Heist Loot Auto-Grab")};
	}
}