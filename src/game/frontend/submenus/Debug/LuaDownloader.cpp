...
static bool DownloadLuaScript(const std::string& enteredUrl, std::string fileName)
{
    ...
    {
        auto menu = std::make_shared<Category>(TR("Lua Downloader"));
        auto downloaderGroup =
            std::make_shared<Group>(TR("Download Lua Scripts"));
        ...
        downloaderGroup->AddItem(std::make_unique<ImGuiItem>([]
        {
            ...
            ImGui::TextWrapped(TR("Download Lua scripts directly into:"));
            ...
            ImGui::InputTextWithHint("##LuaDownloadUrl", TR("GitHub raw link or direct .lua link"), luaUrl, sizeof(luaUrl));
			ImGui::InputTextWithHint("##LuaFileName", TR("File name, for example Script.lua"), luaFileName, sizeof(luaFileName));
            ...
            if (ImGui::Button(TR("Download Lua Script")))
            {
                ...
                if (url.empty())
                {
                    ...
                    statusMessage = TR("Enter a direct Lua script URL.");
                }
                else if (!IsValidLuaFileName(fileName))
                {
                    ...
                    downloadSuccess = false;
                    statusMessage = TR("Enter a valid Lua file name.");
                }
                else
                {
                    {
                    ...
                    FiberPool::Push([url, fileName]
                    {
                        ...
                        if (success)
                        {
							statusMessage = TR("Lua script downloaded successfully.");
                        }
                        else
                        {
							statusMessage = TR("Download failed. Use a direct or GitHub raw link.");
                        }
                    });
                }
            }
            ...
            if (ImGui::Button(TR("Open Scripts Folder")))
            {
                const auto scriptsFolder = GetLuaScriptsFolder();
                ...
                if (!scriptsFolder.empty())
                {
                    ...
                    if (!error)
                    {
						...
						ShellExecuteA(nullptr, TR("open"), folder.c_str(), nullptr, nullptr, SW_SHOWNORMAL);
                    }
                }
            }
