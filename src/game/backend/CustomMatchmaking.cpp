...
namespace YimMenu::Features
{
	...
	BoolCommand _SpoofRegionType{
	    ...
	    TR("Spoofs the region type of the session")};
	ListCommand _RegionType{
	    "mmregiontype",
	    TR("Region Type"),
	    TR("The region to spoof the session to"),
		g_RegionCodes};

	BoolCommand _SpoofLanguage{
	    "mmspooflanguage",
	   TR("Spoof Language"),
	    TR("Spoofs the session language")};
	ListCommand _Language{
	    "mmlanguage",
	    TR("Language"),
	    TR("The language to spoof the session to"),
	    g_LanguageTypes};

	BoolCommand _SpoofPlayerCount{
	    "mmspoofplayercount",
	    TR("Spoof Player Count"),
	    TR("Spoofs the session player count")};
	IntCommand _PlayerCount{
	    "mmplayercount",
	    TR("Player Count"),
	    TR("The player count to spoof the session to"),
	    1,
	    32,
	    25};
	...
	BoolCommand _MultiplexSession{
	    ...
	    TR("Makes your session much more visible to other players by hosting multiple instances of it. High values may fill your session in seconds. Use at your own risk")};
	IntCommand _MultiplexCount{
	    "mmmultiplexsessioncount",
	    TR("Multiplex Session Count"),
	    TR("The number of sessions to advertise for matchmaking"),
	    2,
	    7,
	    5};
}
